...
void testConnPC(void * pvParameters)
{
  ...
  xPacketTestConn.pcBytes[4] = 0x08;
  xPacketTestConn.pcBytes[5] = 0x05;
  xPacketTestConn.pcBytes[6] = 0xff;
  xPacketTestConn.pcBytes[7] = 0x08;
  
  while(testConn == 1)
    {
      write(*psConn, (const unsigned char*)xPacketTestConn.pcBytes, (size_t)xPacketTestConn.usLength);
...
void sendCurCoordCycle(void * pvParameters)
{
...
void board_server_serve(int conn) 
{
  ...
  while(1)
  {
    ...
    switch (recv_buffer[2]){ 
      ...
      case 0x02:
        ...
        for(int i = 0; i < 20; i++)
        {
          ...
          /*unsigned char tempCeckSum[] = {0x00, 0x00, 0x00, 0x00};
          bytes tempCS, transmPack;
          //tempCS = { 4,  (unsigned char*){0x00, 0x00, 0x00, 0x00}};
          transmPack = concatMas(pctWoCheckSum, (bytes){4,  tempCeckSum});
          getAndWritePackLength(&transmPack);
          calcCheckSum(&transmPack);*/
      ...
      case 0xff:
        ...
        {
          testConn = 1;
          xTaskCreate(testConnPC, "testConnPC", configMINIMAL_STACK_SIZE, (void*)&conn, WEBSERVER_THREAD_PRIO, NULL);
        }
        continue;
        break;
      default:
    ...
    /*unsigned char xTempCeckSum[] = {0x00, 0x00, 0x00, 0x00};
    Bytes xTempCS, xTransmPack;
    //tempCS = { 4,  (unsigned char*){0x00, 0x00, 0x00, 0x00}};
    xTransmPack = concatMas(pctWoCheckSum, (Bytes){4,  xTempCeckSum});
    getAndWritePackLength(&xTransmPack);
    calcCheckSum(&xTransmPack);*/
